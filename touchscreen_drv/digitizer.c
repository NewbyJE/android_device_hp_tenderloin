...
void touchscreen_power(int enable)
{
    ...
    TSLOGI("touchscreen_power: enable=%d, ts_state=%d", enable, ts_state);
...
try_again:
		...
		if (rc != 1)
			TSLOGE("TSpower, failed set xres");

		/* Then power on */
        lseek(vdd_fd, 0, SEEK_SET);
        rc = write(vdd_fd, "1", 1);
		if (rc != 1)
			TSLOGE("TSpower, failed to enable vdd");
		...
		if (rc != 1)
			TSLOGE("TSpower, failed to assert wake");

        lseek(xres_fd, 0, SEEK_SET);
        rc = write(xres_fd, "0", 1);
		if (rc != 1)
			TSLOGE("TSpower, failed to reset xres");
		...
		if (rc != 1)
			TSLOGE("TSpower, failed to deassert wake");
		...
		if (rc != 1)
			TSLOGE("TSPower, ioctl1 failed %d errno %d\n", rc, errno);
        ...
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			TSLOGE("TSPower, ioctl2 failed %d errno %d\n", rc, errno);

        i2c_msg.len = 2;
        i2c_buf[0] = 0x30; i2c_buf[1] = 0x0F;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			TSLOGE("TSPower, ioctl3 failed %d errno %d\n", rc, errno);

        i2c_buf[0] = 0x40; i2c_buf[1] = 0x02;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			TSLOGE("TSPower, ioctl4 failed %d errno %d\n", rc, errno);

        i2c_buf[0] = 0x41; i2c_buf[1] = 0x10;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			TSLOGE("TSPower, ioctl5 failed %d errno %d\n", rc, errno);

        i2c_buf[0] = 0x0A; i2c_buf[1] = 0x04;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			TSLOGE("TSPower, ioctl6 failed %d errno %d\n", rc, errno);

        i2c_buf[0] = 0x08; i2c_buf[1] = 0x03;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			TSLOGE("TSPower, ioctl7 failed %d errno %d\n", rc, errno);

        lseek(wake_fd, 0, SEEK_SET);
        rc = write(wake_fd, "1", 1);
		if (rc != 1)
			TSLOGE("TSpower, failed to assert wake again");
		ts_state = 1;
    } else if (ts_state) {
        lseek(vdd_fd, 0, SEEK_SET);
        rc = write(vdd_fd, "0", 1);
		if (rc != 1)
			TSLOGE("TSpower, failed to disable vdd");
...
void init_digitizer_fd(void) {
	...
	if (vdd_fd < 0)
		TSLOGE("TScontrol: Cannot open vdd - %d", errno);
	xres_fd = open("/sys/devices/platform/cy8ctma395/xres", O_WRONLY);
	if (xres_fd < 0)
		TSLOGE("TScontrol: Cannot open xres - %d", errno);
	wake_fd = open("/sys/user_hw/pins/ctp/wake/level", O_WRONLY);
	if (wake_fd < 0)
		TSLOGE("TScontrol: Cannot open wake - %d", errno);
	i2c_fd = open("/dev/i2c-5", O_RDWR);
	if (i2c_fd < 0)
		TSLOGE("TScontrol: Cannot open i2c dev - %d", errno);
